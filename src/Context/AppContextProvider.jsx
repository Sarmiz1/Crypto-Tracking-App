...
export default function AppContextProvider({ children }) {
  ...
  const { data: topCoins, loading: topLoading, error: topError } = useFetch(
  ...
  const { data: globalData, loading: globalLoading, error: globalError } = useFetch(
    "https://api.coingecko.com/api/v3/global"
  );

  // --- Fear & Greed Index ---
  const { data: fearGreedData, loading: fgLoading, error: fgError } = useFetch(
    "https://api.alternative.me/fng/"
  );

  // --- Global Trending Coins from CoinGecko ---
  const { data: trendingData, loading: trendingLoading, error: trendingError } = useFetch(
